...
TEST(HipFileThreadPool, ConstructorUsesHardwareThreadCount)
{
    ThreadPool pool{};
    ASSERT_EQ(pool.threadCount(), ThreadPool::hardwareThreadCount());
}

TEST(HipFileThreadPool, HardwareThreadCountIsNonZero)
{
    ASSERT_GT(ThreadPool::hardwareThreadCount(), 0);
}
...
TEST(HipFileThreadPool, ContextDefaultUsesThreadPool)
{
    ...
    ASSERT_EQ(thread_pool->threadCount(), ThreadPool::hardwareThreadCount());
}
