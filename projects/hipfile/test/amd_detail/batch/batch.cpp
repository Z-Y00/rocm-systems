...
TEST_F(HipFileBatch, CancelOperation)
{
    BatchOperation op = BatchOperation{std::move(io_params), default_mock_buffer, default_mock_file};
...
TEST_F(HipFileBatch, CancelOperationIsIdempotent)
{
    BatchOperation op = BatchOperation{std::move(io_params), default_mock_buffer, default_mock_file};
...
TEST_F(HipFileBatchContext, CancelOperationsCancelsWaitingAndPendingOperations)
{
    ASSERT_NE(context(), nullptr);
    ...
    ASSERT_EQ(waiting_op->get_status(), hipFileCanceled);
...
TEST_F(HipFileBatchContext, CancelOperationsMixedStates)
{
    ...
    ASSERT_NO_THROW(context()->get_status(4, &nr, events.data(), nullptr));
...
TEST_F(HipFileBatchContext, DestroyContextCancelsOutstandingOperations)
{
    ASSERT_NE(context(), nullptr);
