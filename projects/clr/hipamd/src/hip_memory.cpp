...
hipError_t ihipMemcpyBatch(void** dsts, void** srcs, size_t* sizes, size_t count,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    hipError_t status;
    if (srcMemories[i] != nullptr && dstMemories[i] != nullptr) {
      status = ihipMemcpy_validate(amdDev, dstMemories[i], srcMemories[i], sizes[i], dstOffsets[i],
                                   srcOffsets[i]);
    } else if (srcMemories[i] != nullptr) {
      status = ihipMemcpy_validate_memory(amdDev, srcMemories[i], sizes[i], srcOffsets[i],
                                          /*read_write*/ false);
    } else {
      status = ihipMemcpy_validate_memory(amdDev, dstMemories[i], sizes[i], dstOffsets[i],
                                          /*read_write*/ true);
    }
    if (status != hipSuccess) {
      return status;
