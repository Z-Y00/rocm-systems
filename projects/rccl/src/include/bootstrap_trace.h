 ...
 *
 * Per-rank in-memory event ring buffer; binary dump on bootstrap exit.
 * Post-process via scripts/merge_bootstrap_trace.py to Chrome Trace
 * Format JSON for inspection in chrome://tracing or Perfetto.
 *
 * Activation:
 *   NCCL_BOOTSTRAP_TRACE=1
 *   NCCL_BOOTSTRAP_TRACE_DIR=/path/to/dump_dir   (default /tmp/bootstrap_trace)
 *
...
// Dump current thread's buffer to file. Idempotent.
