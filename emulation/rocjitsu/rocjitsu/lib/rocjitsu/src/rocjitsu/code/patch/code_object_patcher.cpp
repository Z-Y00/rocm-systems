...
// These ELF values are not currently exposed through amdgpu_elf.h, but the
// patcher needs them to preserve dynamic relocation metadata when sections move.
...
void shift_relocation_offsets_in_moved_sections(std::vector<uint8_t> &image, const Elf64_Ehdr &ehdr,
                                                ...
                                                uint64_t delta) {
  assert(shdrs.size() == shift_section_vaddr.size() && "section shift map size mismatch");
  if (delta == 0 || ehdr.e_type != kElfTypeDyn)
    return;
  ...
  for (const Elf64_Shdr &relocs : shdrs) {
    if (relocs.sh_type != kShtRela && relocs.sh_type != kShtRel)
      continue;
    if (!image_contains_range(image.size(), relocs.sh_offset, relocs.sh_size))
      continue;

    if (relocs.sh_type == kShtRela) {
